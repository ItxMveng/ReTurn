...
async def list_my_declarations(
    ...
    return await declaration_service.list_declarations(db, current_user.id, skip, limit)
...
async def add_photo(
    ...
    if len(decl.photo_urls) >= 3:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Maximum 3 photos par déclaration")
